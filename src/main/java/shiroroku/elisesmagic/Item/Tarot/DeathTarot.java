		...
		if (event.getSource().getEntity() != null && event.getSource().getEntity() instanceof Player player) {
			if (hasTarot(player, ItemRegistry.death.get())) {
				if (event.getEntityLiving().getMobType() != MobType.UNDEAD) {
					ElisesMagic.LOGGER.debug("TAROT PASSIVE: {} - 50% more damage to non-undead", ItemRegistry.death.get());
					ElisesMagic.LOGGER.debug("From : {}", player);
					ElisesMagic.LOGGER.debug("To : {}", event.getEntityLiving());
					ElisesMagic.LOGGER.debug("Amount : {} to {}", event.getAmount(), event.getAmount() * (1 + boost));
